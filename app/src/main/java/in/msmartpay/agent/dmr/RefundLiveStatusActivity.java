...
public class RefundLiveStatusActivity extends BaseActivity {
    ...
    private void refundLiveStatusRequest() {
        ...
        RetrofitClient.getClient(context).transStatus(request).enqueue(new Callback<RefundLiveStatusResponse>() {
            @Override
    ...
    private void refundTransactionRequest() {
        ...
        request.setSenderId(mobileNumber);
        request.setTransactionRefNo(TranNo);
        RetrofitClient.getClient(context).refundTransaction(request).enqueue(new Callback<MainResponse2>() {
            @Override
    ...
    private void confirmRefundRequest() {
        ...
        RetrofitClient.getClient(context).refundTransactionConfirm(request).enqueue(new Callback<MainResponse2>() {
            @Override
            public void onResponse(Call<MainResponse2> call, retrofit2.Response<MainResponse2> response) {
                pd.dismiss();
                if (response.isSuccessful() && response.body() != null) {
