...
public class ConfirmFlightBooking extends BaseActivity {
    private static final String TAG = ConfirmFlightBooking.class.getSimpleName();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        btn_flight_proceed.setOnClickListener((View.OnClickListener) view -> {
            validation();
            //checkPermissions();
        });
                    ...
                    public void onPermissionsChecked(MultiplePermissionsReport multiplePermissionsReport) {
                        if (multiplePermissionsReport.areAllPermissionsGranted()) {
                            ...
                            if (!locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)) {
                                Util.onGPS(ConfirmFlightBooking.this, 121);
                            } else {
                                latitude = Util.LoadPrefData(getApplicationContext(), Keys.LATITUDE);
                                longitude = Util.LoadPrefData(getApplicationContext(), Keys.LONGITUDE);
                                validation();
                            }
                        }
                        if (multiplePermissionsReport.isAnyPermissionPermanentlyDenied()) {
                            Util.openSettingsDialog(ConfirmFlightBooking.this, 101);
                        }
                    }
