...
public class AddBeneficiaryActivity extends BaseActivity {
    ...
    private void getBankListRequest() {
        if (NetworkConnection.isConnectionAvailable(getApplicationContext())) {
            ...
            RetrofitClient.getClient(getApplicationContext())
                    .getBankListDmr(request).enqueue(new Callback<BankListDmrResponse>() {
                @Override
    ...
    private void verifyAddBeneficiaryRequest() {
        if (NetworkConnection.isConnectionAvailable(getApplicationContext())) {
            ...
            RetrofitClient.getClient(getApplicationContext()).addBeneficiary(request)
    ...
    private void VerifyAccountRequest() {
        if (NetworkConnection.isConnectionAvailable(getApplicationContext())) {
            ...
            pd = ProgressDialogFragment.newInstance("Loading. Please wait...", "Fetching Beneficiary Name...");
            ProgressDialogFragment.showDialog(pd, getSupportFragmentManager());
    ...
    private void getBankDetails() {
        if (NetworkConnection.isConnectionAvailable(getApplicationContext())) {
            ...
            request.setBankCode(Bankcode);
            RetrofitClient.getClient(getApplicationContext())
                    .getBankDetailsDmr(request).enqueue(new Callback<BankDetailsDmrResponse>() {
                @Override
