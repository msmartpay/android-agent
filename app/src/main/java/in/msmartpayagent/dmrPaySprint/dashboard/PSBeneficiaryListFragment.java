...
import in.msmartpayagent.dmrPaySprint.PSAddBeneficiaryActivity;
import in.msmartpayagent.dmrPaySprint.PSImpsNeftActivity;
import in.msmartpayagent.dmrPaySprint.adapter.PSBeneListAdapter;
...
public class PSBeneficiaryListFragment extends BaseFragment implements PSBeneListAdapter.BeneListListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.ps_dmr_money_trans_listview_fragment, container, false);
        ...
        floatingButton.setOnClickListener(v -> {
            Intent floatIntent = new Intent(getActivity(), PSAddBeneficiaryActivity.class);
    ...
    public void onPayNow(SenderBeneList model) {
        Intent intent = new Intent(getActivity(), PSImpsNeftActivity.class);
    ...
    @Override
    public void onAccountVerify(SenderBeneList model) {
    ...
    private void getListBene() {
        ...
        if (details.getBeneList() != null && details.getBeneList().size() > 0) {
            ...
            listView.setAdapter(new PSBeneListAdapter(getActivity(), customerDetails, this));
        } else {
    ...
    private void DeleteBeneRequest(String recipientId) {
        if (NetworkConnection.isConnectionAvailable(context)) {
            ...
            RetrofitClient.getClient(context).deleteBeneficiary(Util.LoadPrefData(context,Keys.DYNAMIC_DMR_VENDOR)+AppMethods.DeleteBene,request).enqueue(new Callback<MainResponse2>() {
                @Override
