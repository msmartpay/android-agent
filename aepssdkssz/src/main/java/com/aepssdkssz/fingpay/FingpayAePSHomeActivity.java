...
public class FingpayAePSHomeActivity extends AppCompatActivity {
    ...
    private String token,globalWadh="";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ssz_aeps_activity_home);
        ...
        ImageView btn_top_back = findViewById(R.id.btn_top_back);
        ...
        if (intent != null) {
            ...
            Utility.saveData(getApplicationContext(), Constants.DEVICE_IMEI, intent.getStringExtra(Constants.DEVICE_IMEI));
            Utility.saveData(getApplicationContext(), Constants.TOKEN, token);
            Utility.saveData(getApplicationContext(), Constants.MERCHANT_ID, merchant_id);
        ...
        btn_back.setOnClickListener(view20 -> {
            ...
            in.putExtra("Message","Back to Home");
            in.putExtra("StatusCode","0");
            FingpayAePSHomeActivity.this.setResult(Activity.RESULT_OK,in);
            finish();
        });
        btn_top_back.setOnClickListener(view20 -> {
            Intent in = new Intent();
            in.putExtra("Message","Back to Home");
            in.putExtra("StatusCode","0");
            FingpayAePSHomeActivity.this.setResult(Activity.RESULT_OK,in);
                                ...
                                if (res != null) {
                                    if (res.getStatus() == 1 && res.getData() != null) {
                                        ...
                                        if(res.getData().isKyc() ) {
                                            ...
                                            Utility.saveData(getApplicationContext(), Constants.PARTNER_LOGO_URL,data.getPartner_logo_url());
                                            Utility.saveData(getApplicationContext(), Constants.PARTNER_NAME,data.getPartner_name());
                                            Utility.saveData(getApplicationContext(), Constants.PARTNER_POWERED_BY,data.getPartner_powered_by());
                                            Utility.saveData(getApplicationContext(), Constants.PARTNER_CONTACT_US,data.getPartner_contact_us());
                                            ...
                                            //SDKUserBiometricDialog.showDialog(getSupportFragmentManager(),"549146031835","819dde44f9d503a9","EKYKF3252613190522175604900I","1367673"); // for test purpose
                                            FingpayAePSBiometricPagerAdapter sectionsPagerAdapter = new FingpayAePSBiometricPagerAdapter(getApplicationContext(), getSupportFragmentManager());
                                            viewPager.setAdapter(sectionsPagerAdapter);
                                            tabs.setupWithViewPager(viewPager);
                                            SDKUserNumberDialog.showDialog(getSupportFragmentManager(),"",Utility.getData(getApplicationContext(),Constants.DEVICE_IMEI));
                                        }
                                    } else if (res.getStatus() == 5) {
                                        //Utility.saveData(getApplicationContext(), Constants.BIOMETRIC_PROVIDERS, Utility.getGson().toJson(res.getData().getBiometric_providers()));
                                        Utility.hideView(ll_screen);
                                        Utility.showView(ll_error);
                                        //Utility.showView(btn_onboard);
                                        FingpayAePSBiometricPagerAdapter sectionsPagerAdapter = new FingpayAePSBiometricPagerAdapter(getApplicationContext(), getSupportFragmentManager());
                                        viewPager.setAdapter(sectionsPagerAdapter);
                                        tabs.setupWithViewPager(viewPager);
                                        SDKUserRegistrationDialog.showDialog(getSupportFragmentManager());
    ...
    private void setPages() {
        FingpayAePSSectionsPagerAdapter sectionsPagerAdapter = new FingpayAePSSectionsPagerAdapter(this, getSupportFragmentManager());
        viewPager.setAdapter(sectionsPagerAdapter);
        tabs.setupWithViewPager(viewPager);
    }
    ...
    public Intent checkBiometricProvider(final String device_type, final String biometricFormat,String wadh) {
        try {
            globalWadh=wadh;
            //final String bioFormat="";getPIDOptions();
            //Utility.toast(getApplicationContext(),getPIDOptions());
            if (device_type.trim().equalsIgnoreCase(this.morpho)) {
                if (this.searchPackageName(this.morpho)) {
                    ...
                    final String pidOptXML = this.createPidOptXML(biometricFormat,wadh);
                    Utility.loge("pidOptXML : ",pidOptXML);
                    if (pidOptXML.equals("")) {
                ...
                if (this.searchPackageName(this.secugen)) {
                    ...
                    final String pidOptXML = this.createSecugenPidXML(biometricFormat,wadh);
                    intent2.putExtra("PID_OPTIONS", pidOptXML);
                ...
                final String pidOptXML = this.createPrecisionPidOptXML(biometricFormat,wadh);
                intent2.putExtra("PID_OPTIONS", pidOptXML);
    ...
    private boolean searchPackageName(final String deviceName) {
        ...
        if (resolveInfoList.isEmpty()) {
            new AlertDialog.Builder(getApplicationContext()).setCancelable(true).setTitle("Message").setMessage(message).setPositiveButton("OK", (DialogInterface.OnClickListener) new DialogInterface.OnClickListener() {
    ...
    protected String createPidOptXML(final String biometricFormat,String wadh) {
        try {
    ...
    protected String createSecugenPidXML(final String biometricFormat,String wadh) {
        try {
            final DocumentBuilderFactory docFactory = DocumentBuilderFactory.newInstance();
            docFactory.setNamespaceAware(true);
            final DocumentBuilder docBuilder = docFactory.newDocumentBuilder();
    ...
    protected String createPrecisionPidOptXML(final String biometricFormat,String wadh) {
        try {
            final String fTypeStr = "2";
            String formatStr;
            if (biometricFormat != null && biometricFormat.length() > 0) {
                    ...
                    if (jsonDeviceInfo.has("additional_info")) {
                        ...
                        if (jsonAdditInfo.get("Param") instanceof JSONObject) {
                            ...
                            if (name.equalsIgnoreCase("serial_number")) {
                                ...
                                final Intent intent = new Intent("in.gov.uidai.rdservice.fp.CAPTURE");
                                intent.setPackage("com.scl.rdservice");
                                final String pidOptXML = this.createPidOptXML(biometricFormat,globalWadh);
                                intent.putExtra("PID_OPTIONS", pidOptXML);
                            ...
                            for (int i = 0; i < jsonArray.length(); ++i) {
                                ...
                                if (name2.equalsIgnoreCase("serial_number")) {
                                    ...
                                    final String pidOptXML2 = this.createPidOptXML(biometricFormat,globalWadh);
