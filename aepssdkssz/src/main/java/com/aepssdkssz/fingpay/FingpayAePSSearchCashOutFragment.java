 ...
 */
public class FingpayAePSSearchCashOutFragment extends Fragment {
    ...
    public static FingpayAePSSearchCashOutFragment newInstance(int index) {
        FingpayAePSSearchCashOutFragment fragment = new FingpayAePSSearchCashOutFragment();
        ...
        try {
            ...
            if (this.validateRequest()) {
                if (this.getActivity() != null) {
                    final Intent intent = ((FingpayAePSHomeActivity) this.getActivity()).checkBiometricProvider(this.device_type, this.biometricFormat,"");
    ...
    public void fingpaySubmitRequest() {
        if (Utility.checkConnection(requireActivity())) {
            ...
            try {
                ...
                JSONObject jsonObjPidData = ((FingpayAePSHomeActivity) requireActivity()).getPieJsonData();
                JSONObject jsonBody = ((FingpayAePSHomeActivity) requireActivity()).getTransactionInfo();
                if (jsonObjPidData != null) {
                    ...
                    if (errCode.equals("0")) {
                        ...
                        if (jsonDeviceInfo.has("additional_info")) {
                            ...
                            deviceSerno = ((FingpayAePSHomeActivity) requireActivity()).getMorphoDeviceSerno();
                        }
                        ...
                        data.setTransaction_type(serviceType);
                        ...
                        data.setDeviceserno(Utility.getData(requireActivity(),Constants.DEVICE_IMEI));

                        //For displaying on receipt
                        Utility.saveData(requireActivity(),Constants.BANK_NAME,selectedBankModel.getBank());
                        Utility.saveData(requireActivity(),Constants.NUMBER,et_customer_mobile.getText().toString());
                        ...
                        SSZAePSRetrofitClient.getClient(requireActivity())
                                .fingpayAepsTransaction(request)
    ...
    public void fingpayFetchBank(String servicetype) {
        if (Utility.checkConnection(requireActivity())) {
            ...
            try {
                ...
                request.setToken(Utility.getData(requireActivity(),Constants.TOKEN));
                request.setMerchant_id(Utility.getData(requireActivity(),Constants.MERCHANT_ID));
                request.setEnvironment(servicetype);




                        SSZAePSRetrofitClient.getClient(requireActivity())
                                .fingpayFetchBank(request)
                                    ...
                                    public void onResponse(@NotNull Call<ValidateUserResponse> call, @NotNull Response<ValidateUserResponse> response) {
                                        ...
                                        try {
                                            if (response.isSuccessful() && response.body() != null) {
                                                ...
                                                if (res != null) {
                                                    if (res.getStatus() == 1 && res.getData().isAllow()) {
                                                        Utility.saveData(requireActivity(), Constants.BANK_LIST_PREFERRED, Utility.getGson().toJson(res.getData().getPreferred_bank_list()));
                                                        Utility.saveData(requireActivity(), Constants.BANK_LIST, Utility.getGson().toJson(res.getData().getBank_list()));
                                                    }
                                                } else {
                                                    Utility.showMessageDialogue(requireActivity(), "" + res.getMessage(), "Fetch Bank");
                                                }
                                            } else {
                                                SSZAePSAPIError error = SSZAePSAPIError.parseError(response, requireActivity());
                                                Utility.showMessageDialogue(requireActivity(), error.message(), "Submit Request");
            ...
            if (requestCode == 1000) {
                ...
                if (pidData != null) {
                    ...
                    try {
                        ...
                        final org.json.JSONObject jsonObjPidData = org.json.XML.toJSONObject(pidData);
    ...
    private void freshData() {
        ...
        ((FingpayAePSHomeActivity) requireActivity()).updatePidJson(null);
        //Utility.setCaptureFingerED(iv_fingerprint, false);
    }
    ...
    private void setTransactionType(String purposeOfPayment){
        if(Constants.CASH_WITHDRAWAL.equalsIgnoreCase(purposeOfPayment)){
            ...
            Utility.showView(rl_amount);
            serviceType="2";
        }else if(Constants.MINI_STATEMENT.equalsIgnoreCase(purposeOfPayment) || Constants.FINGPAY_MS.equalsIgnoreCase(purposeOfPayment)){
            tv_cash_withdraw.setBackgroundResource(R.drawable.ssz_cash_withdrawal_s);
            tv_mini_statement.setBackgroundResource(R.drawable.ssz_mini_statement_selected_s);
            tv_req_balance.setBackgroundResource(R.drawable.ssz_balance_inquiry_s);
            tv_aadhar_pay.setBackgroundResource(R.drawable.ssz_cash_withdrawal_s);
